    ...
    def fit(self):
        ...
        for r in range(self.num_rounds):
            ...
            with torch.no_grad():
                ...
                outputs_me = self.model(self.data.data)
                labels_me = self.data.targets
                predicted_me = outputs_me.round()

                acc = predicted_me.eq(labels_me.view_as(predicted_me)).sum().item()/len(self.data.data)
                zy= P_ZY(self.data.sen_attr,predicted_me)
                p00, p01, p10, p11 =zy

                SPg= p11-p01 #global statistical parity 

                As=['00','01','10','11']
                dists=dit.Distribution(As,zy)
                dists.set_rv_names('ZY')
                I_ZYss=dit.shannon.mutual_information(dists, ['Z'], ['Y'])

            dist=[]
            for i in range(len(self.clients)):
                dist=dist+list(np.asarray(self.clients[i].zy)*self.mc[i])
            dist=np.asarray(dist)
            A1=['000','001','010','011','100','101','110','111']
            Uniq, Red, Syn ,Uniq_S, I_ZS, I_YS, I_ZY, I_ZY_S , CoI = PID(A1,dist)  
            ...
            message = f"[Round: {str(self._round).zfill(4)}] Evaluate global model's performance...!\
                ...
                \n\t=> SP_global {SPg :.4f}\
                \n\t..........................\
                \n\t=> I(Z,Y): {I_ZY:.4f} Global Disparity\
                \n\t=> I(Z,Y)all: {I_ZYss:.4f} Global Disparity\
                \n\t=> I(Z,Y|S): {I_ZY_S:.4f} Local Disparity\
                \n\t=> I(Z;S;Y): {CoI:.4f} Co-inform\
                \n\t...........................\
                \n\t=> I(Z,S): {I_ZS:.4f}\
                \n\t=> I(Y,S): {I_YS:.4f}\
                \n\t...........................\
