...
#All imports here
#if this isnt working, install shap first using 'pip install shap'

def get_dataset(name, save=False, corr_sens=False, alpha=0.5, seed=42, verbose=False):
    """
    ...
    if name == 'adult':
        ...
        def sample_from_gaussian(pos_mean,
            ...
            if corr_sens:
                ...
                for i in range(n):
                    if y[i] == 1:
                        sens_attr[i] = np.random.choice([0, 1], p=[1-alpha, alpha])
                    else:
                        sens_attr[i] = np.random.choice([0, 1], p=[alpha, 1-alpha])
            else:
                # independent sens data
                np.random.seed(seed)
                sens_attr = np.random.binomial(1, 0.5, n)
