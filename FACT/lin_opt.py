...
def model_spec_solve(A, b, A_const, b_const, fm, convhull=True, eps=0.001, verbose=False, plot=False, fairtype='EOd'):
    ...
    print(f'Mfactz= {np.dot(M_const,fct_model)}')
    ...
    print(f'fct_model xc = {np.dot(fct_model,c[0,:])}')
    ...
    if fairtype == 'EOd':
        ...
        constraints += [
            ...
            tpr_x_pos - tpr_pos_2 / fpr_pos_2 * (fpr_x_pos - 1) - 1 >= 0, 
            ...
            tpr_x_neg - tpr_neg_2 / fpr_neg_2 * (fpr_x_neg - 1) - 1 >= 0
        ]
    ...
    if verbose:
        print(prob.value)
        print(z.value)
        print(f'Mz {np.dot(M_const,z.value)}')

    if plot:
        f, ax = plt.subplots()
        from matplotlib.patches import Polygon

        print(f'tpr_pos_1 {tpr_pos_2}')
...
def get_abbrvs():
    ...
    names = ['EqOdd', 'Calibration', 'PosClassBal', 'NegClassBal', 'ClassBal', 'PredEqual', 'EqOpp', 'DemoParity','global_DP','local_DP']
    abbrvs = ['EOd', 'CG', 'PCB', 'NCB', 'CB', 'PE', 'EOp', 'DP','Glb','loc']
    abbrvs_dict = dict()
    for i, n in enumerate(names):
